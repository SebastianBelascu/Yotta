...
export default function EditFooterItemPage({ params }: { params: { id: string } }) {
  ...
  const fetchFooterItem = async () => {
    try {
      const response = await fetch(`/api/footer/${params.id}`);
      if (response.ok) {
        const data: FooterItem = await response.json();
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Error updating footer item');
      }
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Edit Footer Item</h1>
        <p className="text-gray-600 mt-1">Update footer link, social media icon, or company information</p>
      </div>
