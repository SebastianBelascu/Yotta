...
export default function EditBlogPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  interface FormData extends Omit<BlogPost, 'tags'> {
    tags: string[] | string;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Convert tags to proper array format
      let tagsArray: string[] = [];
      if (typeof formData.tags === 'string') {
        tagsArray = formData.tags.split(',').map((tag: string) => tag.trim()).filter((tag: string) => tag !== '');
      } else if (Array.isArray(formData.tags)) {
        tagsArray = formData.tags.filter((tag: string) => tag && tag.trim() !== '');
      }
